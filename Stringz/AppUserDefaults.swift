...
fileprivate let xcodeBundleID = "com.apple.dt.Xcode"

fileprivate func getIBToolPath(_ xcPath: String) -> String {
  return "\(xcPath)/Contents/Developer/usr/bin/ibtool"
}
...
extension UserDefaults {
  ...
  static func loadDefaults() {
    ...
    if !standard.bool(forKey: "stringz.didResetDefaults") {
      clearAll()
      standard.set(true, forKey: "stringz.didResetDefaults")
    }
    ...
    guard
      let savedPath = storyboardXcodePath,
      !Path(getIBToolPath(savedPath)).exists
    else {
      return
    }
    
    let xcodePath = NSWorkspace
      .shared
      .urlForApplication(withBundleIdentifier: xcodeBundleID)?
      .path
    
    guard
      let xcodePath = xcodePath,
      Path(getIBToolPath(xcodePath)).exists
    else {
      Common
        .alert(
...
extension UserDefaults {
  ...
  static var showKeyColumn: Bool {
    ...
    set { standard.set(newValue, forKey: KeyShowKeyColumn) }
  }
  
  static var showCommentColumn: Bool {
    get { standard.bool(forKey: KeyShowCommentColumn) }
    set { standard.set(newValue, forKey: KeyShowCommentColumn) }
  }
  ...
  static var searchType: SearchType {
    ...
    set { standard.set(newValue.rawValue, forKey: KeySearchType) }
  }
  
  static var searchScope: SearchScope {
    get { SearchScope(rawValue: standard.integer(forKey: KeySearchScope))! }
    set { standard.set(newValue.rawValue, forKey: KeySearchScope) }
  }
  ...
  static var importingIgnoredValues: [IgnoredValue] {
    get {
      if let data = standard.data(forKey: KeyImportingIgnoredValues),
         let array = try? NSKeyedUnarchiver.unarchivedObject(ofClasses: [NSArray.self, IgnoredValue.self, NSString.self], from: data) as? [IgnoredValue] {
        return array
      } else {
    ...
    set {
      ...
      standard.set(data, forKey: KeyImportingIgnoredValues)
    }
  }
  ...
  static var storyboardXcodePath: String? {
    ...
    set { standard.set(newValue, forKey: KeyStoryboardXcodePath) }
  }
  ...
  static var plistImportAll: Bool {
    ...
    set { standard.set(newValue, forKey: KeyPlistImportAll) }
  }
  
  static var plistDefaultKeys: [PlistKey] {
    return plistDefaultDictionary.map {
      PlistKey(uuid: UUID().uuidString, name: $0.key, friendlyName: $0.value)
    }
  }
  ...
  static var plistKeys: [PlistKey] {
    get {
      if let data = standard.data(forKey: KeyPlistKeys),
         let array = try? NSKeyedUnarchiver.unarchivedObject(ofClasses: [NSArray.self, PlistKey.self, NSString.self], from: data) as? [PlistKey] {
        return array
      } else {
