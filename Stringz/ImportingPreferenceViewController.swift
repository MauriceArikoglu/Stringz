...
final class ImportingPreferenceViewController: PreferenceViewController, PreferencePane {
  ...
  @objc dynamic var sortDescriptors = [
    NSSortDescriptor(key: "name", ascending: true) {
      ($0 as! String).localizedCaseInsensitiveCompare($1 as! String)
    },
  ]
...
extension ImportingPreferenceViewController: NSTableViewDelegate, NSTextFieldDelegate {
  ...
  func control(_ control: NSControl, textShouldEndEditing fieldEditor: NSText) -> Bool {
    // A stupid workaround because this method is being called twice with empty value in the second time
    guard
      shouldAcceptEditing
    else {
    ...
    if identifier == "name" {
      guard
        !self.ignoredValues.contains(where: { $0.name == newValue })
      else {
        return false
      }
